...
%
	...
	Es wird je ein Ultraschall-Modul vorne und eines hinten am Roboter montiert. Sie müssen nur an den \iic-Bus angeschlossen werden und benötigen keine weitere Hardware. Die Infrarotsensoren werden auch vorne und hinten, je nach Platz positioniert. Für diese Sensoren ist eine einfache Schaltung notwendig. Die zwei LEDs werden über ein Trimmpotentiometer am Sensor angeschlossen, so kann gegebenenfalls die Schaltdistanz herabgesetzt werden.
	...
	\end{description}
	%
	Damit das Infrarotlicht nicht direkt in den Sensor strahlt sind Schrumpfschläuche um die LEDs notwendig. Es muss jeweils darauf geachtet werden, dass zwischen Print und Schrumpfschlauch kein Spalt ist durch den das Infrarotlicht scheinen kann. Die optimale Methode zum Anpassen der Schrumpfschläuche ist Folgende: \SI{1}{\centi\meter} abschneiden und um einen \SI{5}{\milli\meter} Inbusschlüssel schrumpfen. Danach kann der Schrumpfschlauch einfach auf die LEDs gesteckt werden und sollte so ideal befestigt sein.
	%
	\par Es wurde auch festgestellt, dass keine roten und blauen Schrumpfschläuche verwendet werden können, vermutlich kann das Infrarotlicht durch diese hindurch scheinen wodurch der Sensor immer LOW (also "`Objekt detektiert"') ausgibt. Mit schwarzen und braunen Schrumpfschläuchen funktioniert der Aufbau. Alle anderen Varianten wurden nicht getestet.
	%
	...
	%
		...
		Damit am gleichen \iic-Bus beide Ultraschallsensoren verwendet werden können, musste zuerst eine separate Software zum ändern der Slave-Adresse erstellt werden. Der Ablauf ist der folgende:
		\begin{enumerate}
			\item Überprüfgen ob neue Adresse gültig ist. Falls nicht, Programm nicht fortsetzten.
			\item Erste Prüfsequenz zur Adressänderung via \iic\ ans Kommandoregister senden und definierte Zeit warten.
			\item Zweite Prüfsequenz zur Adressänderung via \iic\ ans Kommandoregister senden und definierte Zeit warten.
			\item Dritte Prüfsequenz zur Adressänderung via \iic\ ans Kommandoregister senden und definierte Zeit warten.
			\item Neue Adresse via \iic\ ans Kommandoregister senden.
			\item Beim Aus- und erneuten Einstecken sollte nun die LED auf dem Modul mit dem Code für die neue Adresse (siehe Spezifikationen \cite{url:spec_srf08}) blinken.
		\end{enumerate}
		%
		...
		%
			...
			Da pro Infrarotsensor lediglich ein GPIO\footnote{General purpose input/output} benötigt wird, ist der Taskablauf schnell zusammengefasst:
			\begin{enumerate}
			...
			%
				...
				Als maximale Reichweite wird in dieser Version \SI{1}{\meter} verwendet. Im Header-File\footnote{"`Rangefinder/src/application/Rangefinder.h"'} ist dafür ein Makro\footnote{"`RANGEFINDER\_RANGE"'} definiert mit dem der Wert schnell angepasst werden kann.
				%
				\par Der Registerwert der \textbf{Verstärkung} kann durch Probieren auf die Anwendung angepasst werden. In der Funktion wird wie bei der Reichweite zuerst der übergebene Wert überprüft, gegebenenfalls angepasst und dann an das Modul gesendet.
				%
	...
	%Für die Verkabelung wird ein Flachbandkabel mit zehn Leitern verwendet. Die Stecker sind folgendermassen angeordnet:
	%\todo{text}